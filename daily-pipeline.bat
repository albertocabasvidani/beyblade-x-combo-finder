...
REM Un solo task: l'ordine conta (parts -> collect -> analyze -> mine reddit), niente parallele.
...
set REDDIT_HEADED=1
set WBO_HEADED=1
echo === 1/4 Aggiornamento parti ===
claude --dangerously-skip-permissions -p "Esegui /update-parts"
echo.
echo === 2/4 Raccolta fonti (MetaBeys/YouTube/Sheets + Reddit/WBO headed) ===
call npm run collect:sources
echo.
echo === 3/4 Analisi e aggiornamento combo (fonti strutturate) ===
claude --dangerously-skip-permissions -p "Esegui /update-combos"
echo.
echo === 4/4 Mining Reddit a blocchi (tutta la cache, idempotente) ===
