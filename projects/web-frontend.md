...
- 11/09/2026 — **Monetizzazione e analytics** (piano `plans/monetizzazione-analytics-filtro-periodo-2026-09-11-1500.md`): fatte le fasi 1-6 nel codice. Resta: riattivazione `/update-combos` sul server (F7) e giro col browser MCP sul sito pubblicato (F8.2). **Lato utente**: (1) registrare beybladexcombos.com, DNS e Custom domain su GitHub Pages, poi il «giorno del passaggio» descritto in CLAUDE.md (CNAME + due default in `astro.config.mjs`); (2) chiave PostHog in `src/lib/analytics-config.ts` (oggi placeholder → analytics spenta); (3) tracking ID dedicati nei 6 marketplace di `data/amazon-config.json` (oggi vuoti → link senza tag) e sito aggiunto alle proprietà Associates; (4) domanda AdSense col dominio a sito online, poi `src/lib/ads-config.ts`, `public/ads.txt`, messaggio GDPR da Privacy & messaging; (5) `npm run sync:amazon-asins` sul server dentro `/update-parts`
